...
import br.com.flowlife.services.MedicoService;
import br.com.flowlife.services.mapper.MedicoMapper;
...
public class MedicoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<MedicoDTO> buscarMedico(@Valid @PathVariable Long id) {
    ...
    @GetMapping("/page")
    public ResponseEntity<List<MedicoDTO>> listarMedicoPage(Pageable pageable) {
        List<MedicoDTO> listarMedico = medicoService.procurarMedicoPage(pageable).stream().map(MedicoMapper::toDTO).toList();
        return ResponseEntity.ok(listarMedico);
    }
